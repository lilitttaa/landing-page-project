...
import { Button, Dialog, DialogContent, DialogTrigger } from "../common";
    ...
    return (<EditProvider isEditMode={isEditMode} onUpdate={handleUpdate} basePath={basePath}><section id="relume" className="px-[5%] py-16 md:py-24 lg:py-28">
      <div className="container">
        <div className="grid grid-cols-1 gap-x-20 gap-y-12 md:gap-y-16 lg:grid-cols-2 lg:items-center">
          <div>
            <EditableText as="h1" path="heading" value={heading} className="mb-5 text-6xl font-bold md:mb-6 md:text-9xl lg:text-10xl">{heading}</EditableText>
          ...
          <Dialog>
            <DialogTrigger asChild>
              <button className="relative flex w-full items-center justify-center">
                <EditableImage src={image.src} path="image" alt={image.alt} className="size-full object-cover" />
        ...
        })} src={video} allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen onLoad={() => setIsIframeLoaded(true)}></iframe>
            </DialogContent>
          </Dialog>
        </div>
      </div>
